...
export function AnimatedBackground() {
  ...
  useEffect(() => {
    ...
    class Particle {
      ...
      constructor(width: number, height: number) {
        ...
        this.size = Math.random() * 2.2 + 0.9; // more visible
      ...
      update(width: number, height: number) {
        ...
        if (this.opacity < 0.15 || this.opacity > 0.8) this.fade *= -1;
      }
    ...
    const animate = () => {
      ...
      for (let i = 0; i < particles.length; i++) {
        ...
        for (let j = i + 1; j < particles.length; j++) {
          ...
          if (distSq < 14000) {
            const opacity = 0.28 * (1 - Math.sqrt(distSq) / 120);
  ...
  return (
    <canvas
      ref={canvasRef}
      className="fixed inset-0 pointer-events-none opacity-40 dark:opacity-40"
      style={{ zIndex: 0 }}
    />
  );
}
